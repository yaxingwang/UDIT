...
class UDIT(object) :
            ...
            for idx in range(start_batch_id, self.iteration):
                ...
                if np.mod(idx+1, self.print_freq) == 0 :
                    ...
                    save_images(batch_B_images, [self.batch_size, 1],
                                '{}/real_B_{}_{:02d}_{:06d}.jpg'.format(self.sample_dir, self.gpu_id, epoch, idx+1))

                    save_images(fake_A, [self.batch_size, 1],
                                '{}/fake_A_{}_{:02d}_{:06d}.jpg'.format(self.sample_dir, self.gpu_id, epoch, idx+1))
    ...
    def test(self):
        ...
        test_A_files = ['dataset/{}/'.format(self.dataset_name + '/testA') + '%d.jpg'%(i+1) for i in xrange(100)] 
        ...
        self.saver.restore(self.sess, '/checkpoint/UDIT_black_red_bias_90_10_percent_lsgan_flat_texture/UDIT.model-16001')
        ...
        for sample_file  in test_A_files : # A -> B
            ...
            img_set = np.zeros((self.img_h, self.img_w*(self.num_style + 1), 3))
            img_set[:, 0:self.img_w, :] =sample_image[0].copy() 
            ...
            for i in range(self.num_style) :
                ...
                image_path = os.path.join(self.result_dir, '{}.{}'.format(img_number, file_extension))

                fake_img = self.sess.run(self.test_fake_B, feed_dict = {self.test_image : sample_image, self.test_style : test_style})
                img_set[:, (i+1)*self.img_w:(i + 2)*self.img_w, :] =fake_img[0].copy() 
            ...
            img_set_path = os.path.join(self.result_dir, 'in_out/{}_in_out.{}'.format(file_name, file_extension))
            Imsave(img_set_path , (img_set + 1.)*127.5)
