...
def make_generator(path, batch_size, image_size, pharse='train', domain = None, start_point = 1):
    ...
    def get_epoch():
	    ...
	    #label = scipy.misc.imresize(label,(image_size/2,image_size), interp = 'nearest')
            ...
            labels_raw[n % batch_size] = label_raw
            
            label_twin_raw[label_twin_raw != -1] = 1.
            labels_twin_raw[n % batch_size] = label_twin_raw

            if n > 0 and n % batch_size == 0:
                yield (images,labels,labels_raw,images_twin, labels_twin,labels_twin_raw)
    return get_epoch
