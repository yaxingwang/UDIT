...
def make_generator(path, batch_size, image_size, pharse='train'):
    epoch_count = [1]
    images_path = [line.strip() for  line in open('./tflib/cityscapes_dataname/%s.txt'%pharse, 'r')] 
    images_list = []
    labels_list = []
    for image_path  in images_path:
        images_list.extend([path + '/leftImg8bit/%s/'%pharse+ image_path])
    if pharse == 'val':
        labels_path = [line.strip() for  line in open('./tflib/cityscapes_dataname/label.txt', 'r')] 
...
if __name__ == '__main__':
    train_gen, valid_gen = load(1, data_dir='/home/yaxing/data/leftImg8bit_trainvaltest',image_size = 1024)
    t0 = time.time()
    pdb.set_trace()
